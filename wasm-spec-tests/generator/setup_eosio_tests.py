    ...
    for d in os.listdir():
        ...
        if wasm_file in altered_wasms:
            ...
            try:
                ...
                out = subprocess.run(
                    ['vectrum-wast2wasm', o_wast_file, '-o', os.path.join(d, wasm_file)],
                    capture_output=True
                )
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("""Please provide:
                ...
                ex:
                python setup_eosio_tests.py ~/code/vectrum-vm-test-wasms ~/code/vectrum ~/wasm_spec_tests
              """)
        sys.exit(1)
    ...
    generator = os.path.join(GENERATOR_DIR, 'vectrum_test_generator')
