...
static constexpr uint64_t header_id = 0x31434f4d564d5456ULL; //"VTMVMOC1" little endian
...
{
   FC_ASSERT(_threads, "VECTRUM VM OC requires at least 1 compile thread");
      ...
      if(_outstanding_compiles_and_poison[result.code] == false) {
         result.result.visit(overloaded {
            ...
            [&](const compilation_result_unknownfailure&) {
               wlog("code ${c} failed to tier-up with VECTRUM VM OC", ("c", result.code.code_id));
   ...
   if(_outstanding_compiles_and_poison.size()) {
      ...
      while(count_processed && _queued_compiles.size()) {
         ...
         if(codeobject) {
            ...
            FC_ASSERT(write_message_with_fds(_compile_monitor_write_socket, compile_wasm_message{ *nextup }, fds_to_pass), "VECTRUM VM failed to communicate to OOP manager");
            --count_processed;
         }
...
code_cache_sync::~code_cache_sync() {
   ...
   auto [success, message, fds] = read_message_with_fds(_compile_monitor_read_socket);
   if(success)
      elog("unexpected response from VECTRUM VM OC compile monitor during shutdown");
}
   ...
   _cache_file_path(data_dir/"code_cache.bin")
{
   ...
   if(!bfs::exists(_cache_file_path)) {
      ...
      EOS_ASSERT(ofs.good(), database_exception, "unable to create VECTRUM VM Optimized Compiler code cache");
   ...
   EOS_ASSERT(cache_header.id == header_id, bad_database_version_exception, "existing VECTRUM VM OC code cache not compatible with this version");
   ...
   if(cache_header.serialized_descriptor_index) {
      ...
      ilog("VECTRUM VM Optimized Compiler code cache loaded with ${c} entries; ${f} of ${t} bytes free", ("c", number_entries)("f", allocator->get_free_memory())("t", allocator->get_size()));
   }
