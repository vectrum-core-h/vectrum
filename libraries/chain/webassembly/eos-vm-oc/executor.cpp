...
#error VECTRUM VM OC is not compatible with Clang ShadowCallStack
#endif
#endif
...
executor::executor(const code_cache_base& cc) {
   ...
   if(arch_prctl(ARCH_GET_GS, &current_gs) || current_gs)
      wlog("x86_64 GS register is not set as expected. VECTRUM VM OC may not run correctly on this platform");
