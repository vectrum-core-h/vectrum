...
This section describes how to set up a producing node within the VECTRUM network. A producing node, as its name implies, is a node that is configured to produce blocks in an `VECTRUM` based blockchain. This functionality if provided through the `producer_plugin` as well as other [vectrum-node Plugins](../../03_plugins/index.md).
...
```sh
vectrum-cli system regproducer accountname1 EOS1234534... http://producer.site Antarctica
```
...
* `public-key` - A valid VECTRUM public key in form of a string.
...
#### Using vectrum-wallet:
You can also use `vectrum-wallet` instead of hard-defining keys. 
