...
```sh
# command-line
vectrum-node ... --plugin eosio::trace_api_plugin [options]
```
...
Here is a `vectrum-node` configuration example for the `trace_api_plugin` when tracing some VECTRUM reference contracts:

```sh
vectrum-node --data-dir data_dir --config-dir config_dir --trace-dir traces_dir
--plugin eosio::trace_api_plugin 
