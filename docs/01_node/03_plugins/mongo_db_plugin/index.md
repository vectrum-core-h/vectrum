...
The optional `eosio::mongo_db_plugin` provides archiving of blockchain data into a MongoDB. It is recommended that the plugin be added to a non-producing node as it is designed to shut down on any failed insert into the MongoDB and it is resource intensive. For best results dedicate a `vectrum-node` instance to running this one plugin. The rationale behind this shutdown on error is so that any issues with connectivity or the mongo database can be fixed and `vectrum-node` can be restarted without having to resync or replay.
...
* Action data is stored on chain as raw bytes. This plugin attempts to use associated ABI on accounts to deserialize the raw bytes into expanded `abi_def` form for storage into mongo. Note that invalid or missing ABI on a contract will result in the action data being stored as raw bytes. For example the VECTRUM system contract does not provide ABI for the `onblock` action so it is stored as raw bytes.
...
* It is recommended that a large `--abi-serializer-max-time-ms` value be passed into the `vectrum-node` running the `mongo_db_plugin` as the default ABI serializer time limit is not large enough to serialize large blocks.
* Read-only mode should be used to avoid speculative execution. See [vectrum-node Read Modes](../../02_usage/05_node-implementation.md#vectrum-node-read-modes). Forked data is still recorded (data that never becomes irreversible) but speculative transaction processing and signaling is avoided, minimizing the transaction_traces/action_traces stored.
...
```console
  -q [ --mongodb-queue-size ] arg (=256)
                                        The target queue size between vectrum-node
  ...
  -m [ --mongodb-uri ] arg              MongoDB URI connection string, see:
                                        ...
                                        database 'VECTRUM' is used if not specified
