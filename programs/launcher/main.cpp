...
string producer_names::producer_name(unsigned int producer_number, bool shared_producer) {
   ...
   if (string(prod_name) == "defproducera" && producer_number != 0)
      throw std::runtime_error( "vectrum-launcher not designed to handle numbers this large " );
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  char* erd_env_var = getenv ("VECTRUM_HOME");
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable VECTRUM_HOME is set to the build path." << endl;
    exit (-1);
  }
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/vectrum-tn_bounce.sh " + eosd_extra_args;
      ...
      do_command(host, node.name, { { "VECTRUM_HOME", host.eosio_home }, { "VECTRUM_NODE", node_num } }, cmd);
   }
}
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/vectrum-tn_down.sh ";
      do_command(host, node.name,
                 { { "VECTRUM_HOME", host.eosio_home }, { "VECTRUM_NODE", node_num }, { "VECTRUM_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
...
launcher_def::roll (const string& host_names) {
   ...
   for (string host_name: hosts) {
      ...
      string cmd = "./scripts/vectrum-tn_roll.sh ";
      do_command(*host, host_name, { { "VECTRUM_HOME", host->eosio_home } }, cmd);
   }
}
...
int main (int argc, char *argv[]) {
  ...
  variables_map vmap;
  options_description cfg ("Testnet vectrum-launcher config options");
  options_description cli ("vectrum-launcher command line options");
  launcher_def top;
  string gts;
  launch_modes mode;
  string kill_arg;
  ...
  cli.add_options()
    ...
    ("kill,k", bpo::value<string>(&kill_arg),"The vectrum-launcher retrieves the previously started process ids and issues a kill to each.")
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the vectrum-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the vectrum-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the vectrum-tn_roll.sh script")
