/**
  ...
  @section intro Introduction to vectrum-cli

  `vectrum-cli` is a command line tool that interfaces with the REST api exposed by @ref vectrum-node. In order to use `vectrum-cli` you will need to
  have a local copy of `vectrum-node` running and configured to load the 'eosio::chain_api_plugin'.

   vectrum-cli contains documentation for all of its commands. For a list of all commands known to vectrum-cli, simply run it with no arguments:
```
$ ./vectrum-cli
Command Line Interface to vectrum-cli
Usage: programs/cli/vectrum-cli [OPTIONS] SUBCOMMAND
...
Options:
  ...
  --wallet-url TEXT=http://localhost:8888/
                              the http/https URL where vectrum-wallet is running
...
//copy pasta from vectrum-wallet's main.cpp
...
string default_wallet_url = "unix://" + (determine_home_directory() / "vectrum-wallet" / (string(wallet_executable_name) + ".sock")).string();
...
void ensure_keosd_running(CLI::App* app) {
    if (no_auto_keosd)
        return;
    // get, version, net, convert do not require vectrum-wallet
    ...
    // This extra check is necessary when running vectrum-cli like this: ./vectrum-cli ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(wallet_executable_name); // if vectrum-cli and vectrum-wallet are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("vectrum-wallet").append(wallet_executable_name);
    }
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->callback([this] {
            auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so vectrum-cli can still work with old buggy vectrum-node versions
                               // Change to voter.value when vectrum-cli no longer needs to support vectrum-node versions older than 1.5.0
                               ("limit", 1)
            );
            auto res = result.as<eosio::chain_apis::read_only::get_table_rows_result>();
            // Condition in if statement below can simply be res.rows.empty() when vectrum-cli no longer needs to support vectrum-node versions older than 1.5.0
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so vectrum-cli can still work with old buggy vectrum-node versions
                               // Change to newname.value when vectrum-cli no longer needs to support vectrum-node versions older than 1.5.0
                               ("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
   ...
   if (!json_format) {
      ...
      if ( res.total_resources.is_object() ) {
         ...
         if( net_total.get_symbol() != unstaking.get_symbol() ) {
            // Core symbol of vectrum-node responding to the request is different than core symbol built into vectrum-cli
...
int main( int argc, char** argv ) {
   ...
   CLI::App app{"Command Line Interface to vectrum-cli"};
   ...
   getCode->callback([&] {
      ...
      try {
         ...
         //see if this is an old vectrum-node that doesn't support get_raw_code_and_abi
...
//                                ->check(CLI::ExistingFile);
   ...
   stopKeosd->callback([] {
      ...
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success vectrum-wallet responds with empty object
   ...
   review->callback([&] {
      const auto result1 = call(get_table_func, fc::mutable_variant_object("json", true)
                                 ...
                                 // Less than ideal upper_bound usage preserved so vectrum-cli can still work with old buggy vectrum-node versions
                                 // Change to name(proposal_name).value when vectrum-cli no longer needs to support vectrum-node versions older than 1.5.0
                                 ("limit", 1)
                           );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;

      const auto& rows1 = result1.get_object()["rows"].get_array();
      // Condition in if statement below can simply be rows.empty() when vectrum-cli no longer needs to support vectrum-node versions older than 1.5.0
