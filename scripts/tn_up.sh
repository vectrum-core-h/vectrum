#!/bin/bash
#
# vectrum-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# vectrum-tn_bounce.sh and vectrum-tn_roll.sh scripts.
...
datadir=var/lib/node_$VECTRUM_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/vectrum/node_$VECTRUM_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/vectrum/node_$VECTRUM_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
...
if [ -z "$VECTRUM_LEVEL" ]; then
    echo starting with no modifiers
    relaunch
    if [ "$connected" -eq 0 ]; then
        VECTRUM_LEVEL=replay
    else
        exit 0
    fi
fi

if [ "$VECTRUM_LEVEL" == replay ]; then
    echo starting with replay
    relaunch --hard-replay-blockchain
    if [  "$connected" -eq 0 ]; then
        VECTRUM_LEVEL=resync
    else
        exit 0
    fi
fi
if [ "$VECTRUM_LEVEL" == resync ]; then
