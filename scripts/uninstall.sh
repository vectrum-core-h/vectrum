...
function usage() {
  ...
  -i DIR      Directory where VECTRUM is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the VECTRUM data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
...
export VECTRUM_INSTALL_DIR=${INSTALL_LOCATION:-$VECTRUM_INSTALL_DIR}

if [[ ! -d "${VECTRUM_INSTALL_DIR}" ]]; then
   echo "[VECTRUM installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${VECTRUM_INSTALL_DIR}]"
else
   # As of 1.8.0, we're using a versioned directories under home
   echo "[VECTRUM installation found: ${VECTRUM_INSTALL_DIR}]" && INSTALL_PATHS+=("${VECTRUM_INSTALL_DIR}") # VECTRUM_INSTALL_DIR set in .environment
   while true; do
      [[ $NONINTERACTIVE == false ]] && read -p "Do you wish to remove the installation in ${VECTRUM_INSTALL_DIR}? (y/n) " PROCEED
      case $PROCEED in
         "" ) echo "What would you like to do?";;
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/vectrum")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/vectrum")
            fi
            # Version < 1.8.0; Before we started using ~/vectrum/1.8.x
            ...
            echo " - VECTRUM Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled VECTRUM Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing VECTRUM Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
