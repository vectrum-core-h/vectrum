...
if [[ -z "${NAME}" ]]; then
    ...
    else echo " ${COLOR_RED}- VECTRUM is not supported for your Architecture!${COLOR_NC}" && exit 1
    fi
fi

# Setup yum and apt variables
if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
    if ! YUM=$( command -v yum 2>/dev/null ); then echo "${COLOR_RED}YUM must be installed to compile VECTRUM${COLOR_NC}" && exit 1; fi
elif [[ $NAME == "Ubuntu" ]]; then
    if ! APTGET=$( command -v apt-get 2>/dev/null ); then echo "${COLOR_RED}APT-GET must be installed to compile VECTRUM${COLOR_NC}" && exit 1; fi
fi
...
function setup() {
    if $VERBOSE; then
        ...
        echo "VECTRUM_INSTALL_DIR: ${VECTRUM_INSTALL_DIR}"
...
function ensure-which() {
  if ! which ls &>/dev/null; then
    while true; do
      [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}VECTRUM compiler checks require the 'which' package: Would you like for us to install it? (y/n)?${COLOR_NC}" && read -p " " PROCEED
      echo ""
      case $PROCEED in
...
function install-directory-prompt() {
    if [[ -z $INSTALL_LOCATION ]]; then
        echo "No installation location was specified. Please provide the location where VECTRUM is installed."
        while true; do
            [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to use the default location? ${VECTRUM_INSTALL_DIR}? (y/n)${COLOR_NC}" && read -p " " PROCEED
            echo ""
            case $PROCEED in
                "" )
                echo "What would you like to do?";;
                ...
                break;;
                1 | false | [Nn]* )
                printf "Enter the desired installation location." && read -p " " VECTRUM_INSTALL_DIR;
                export VECTRUM_INSTALL_DIR;
                break;;
                * ) echo "Please type 'y' for yes or 'n' for no.";;
            esac
        done
        ...
        export VECTRUM_INSTALL_DIR="$INSTALL_LOCATION"
    fi
    . ./scripts/.build_vars
    echo "VECTRUM will be installed to: ${VECTRUM_INSTALL_DIR}"
}

function previous-install-prompt() {
  if [[ -d $VECTRUM_INSTALL_DIR ]]; then
    echo "VECTRUM has already been installed into ${VECTRUM_INSTALL_DIR}... It's suggested that you ./uninstall.sh before re-running this script."
    while true; do
...
function resources() {
    echo "${COLOR_CYAN}VECTRUM website:${COLOR_NC} https://vectrum.group"
}

function print_supported_linux_distros_and_exit() {
   echo "On Linux the VECTRUM build script only supports Amazon, Centos, and Ubuntu."
...
function ensure-cmake() {
    ...
    if [[ ! -e "${CMAKE}" ]]; then
        ...
        && ./bootstrap --prefix=${VECTRUM_INSTALL_DIR} \
        && make -j${JOBS} \
        && make install \
        && cd .. \
...
function ensure-boost() {
    ...
    if [[ "${BOOSTVERSION}" != "${BOOST_VERSION_MAJOR}0${BOOST_VERSION_MINOR}0${BOOST_VERSION_PATCH}" ]]; then
        ...
        execute bash -c "cd $SRC_DIR && \
        curl -OL https://boostorg.jfrog.io/artifactory/main/release/$BOOST_VERSION_MAJOR.$BOOST_VERSION_MINOR.$BOOST_VERSION_PATCH/source/boost_$BOOST_VERSION.tar.bz2 \
