#!/bin/bash
#
# vectrum-tn_roll is used to have all of the instances of the vectrum-node on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: vectrum-tn_roll.sh [arglist]
...
# the command line.
#
# In most cases, simply running ./vectrum-tn_roll.sh is sufficient.
#

if [ -z "$VECTRUM_HOME" ]; then
    echo VECTRUM_HOME not set - $0 unable to proceed.
    exit -1
fi
...
if [ $DD = "all" ]; then
    for VECTRUM_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $VECTRUM_HOME/scripts/vectrum-tn_up.sh "$*"
    done
else
    bash $VECTRUM_HOME/scripts/vectrum-tn_up.sh "$*"
fi
unset VECTRUM_RESTART_DATA_DIR
