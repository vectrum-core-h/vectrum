...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="vectrum-ignition-wd/bootlog.txt"

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is vectrum-wallet running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="VECTRUMtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet vectrum-wallet running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: VECTRUM server host
        port: VECTRUM server port
        walletHost: VECTRUM wallet host
        walletPort: VECTRUM wallet port
        enableMongo: Include mongoDb support, configures VECTRUM mongo plugin
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
        ...
        extraNodeArgs: string of arguments to pass through to each nodoes instance (via --vectrum-node flag on launcher)
        ...
        specificExtraNodeArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-vectrum-node flags on launcher), example: { "5" : "--plugin eosio::test_control_api_plugin" }
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on vectrum-node default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        if specificExtraNodeArgs is not None:
            ...
            for nodeNum,arg in specificExtraNodeArgs.items():
                ...
                cmdArr.append("--specific-vectrum-node")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-vectrum-node")
                cmdArr.append(path)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on VECTRUM server port %d" % (
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on VECTRUM node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        receiving transferAmount*n VTM and forwarding x-transferAmount funds. Transfer actions are spread round-robin across the cluster to vaidate system cohesiveness."""
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse VECTRUM private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by vectrum-launcher."""
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull, env=env):
            if not silent: Utils.Print("Launcher failed to shut down VECTRUM cluster.")
            return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        """Create 'prodCount' init accounts and deposits 10000000000 VTM in each. If prodCount is -1 will initialize all possible producers.
    ...
    # Kills a percentange of VECTRUM instances starting from the tail and update InstanceInfos state
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster vectrum-node instances. allInstances will kill all vectrum-node instances running on the system."""
        ...
        if allInstances:
            # ocassionally the launcher cannot kill the VECTRUM server
            cmd="pkill -9 %s" % (Utils.NodeName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down VECTRUM cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces vectrum-node instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down vectrum-node instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
