...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, walletMgr=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="VECTRUMtest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def currencyStrToInt(balanceStr):
        """Converts currency string of form "12.3456 VTM" to int 123456"""
    ...
    def currencyIntToStr(balance, symbol):
        """Converts currency int of form 123456 to string "12.3456 VTM" where VTM is symbol string"""
    ...
    def validateFunds(self, initialBalances, transferAmount, source, accounts):
        """Validate each account has the expected VTM balance. Validate cumulative balance matches expectedTotal."""
        assert(source)
        assert(isinstance(source, Account))
        assert(accounts)
        assert(isinstance(accounts, list))
    ...
    # Get actions mapped to an account (vectrum-cli get actions)
    ...
    def getAccountBalanceStr(self, scope):
        """Returns VTM currency0000 account balance from vectrum-cli get table command. Returned balance is string following syntax "98.0311 VTM". """
        assert isinstance(scope, str)
    ...
    def getAccountBalance(self, scope):
        """Returns VTM currency0000 account balance from vectrum-cli get table command. Returned balance is an integer e.g. 980311. """
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by vectrum-cli get info."""
        if not self.enableMongo:
    ...
    # If nodePath is equal to None, it will use the existing vectrum-node path
