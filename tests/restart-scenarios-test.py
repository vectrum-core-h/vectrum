...
#
# Tests restart scenarios for vectrum-node.  Uses "-c" flag to indicate "replay" (--replay-blockchain), "resync"
# (--delete-all-blocks), "hardReplay"(--hard-replay-blockchain), and "none" to indicate what kind of restart flag should
# be used. This is one of the only test that actually verify that vectrum-node terminates with a good exit status.
#
###############################################################
...
try:
    ...
    if cluster.killSomeInstances(killCount, killSignal) is False:
        errorExit("Failed to kill VECTRUM instances")
    Print("vectrum-node instances killed.")
    ...
    if cluster.relaunchInstances(cachePopen=True) is False:
        errorExit("Failed to relaunch VECTRUM instances")
    Print("vectrum-node instances relaunched.")
    ...
    if killInstances:
        ...
        assert atLeastOne, "Test is setup to verify that a cleanly interrupted vectrum-node exits with an exit status of 0, but this test may no longer be setup to do that"
