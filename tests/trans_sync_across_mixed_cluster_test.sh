...
getHeadBlockNum()
{
  INFO="$(programs/cli/vectrum-cli get info)"
  verifyErrorCode "vectrum-cli get info"
...
# stand up VECTRUM cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --vectrum-node \"--plugin eosio::wallet_api_plugin\" $launcherOpts
programs/launcher/vectrum-launcher --vectrum-node "--plugin eosio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/cli/vectrum-cli --port $port2 get block 1)"
    verifyErrorCode "vectrum-cli get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/cli/vectrum-cli create key)"
verifyErrorCode "vectrum-cli create key"
...
PASSWORD_INITA="$(programs/cli/vectrum-cli wallet create --name inita)"
verifyErrorCode "vectrum-cli wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/cli/vectrum-cli wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "vectrum-cli wallet import"
programs/cli/vectrum-cli wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "vectrum-cli wallet import"
programs/cli/vectrum-cli wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "vectrum-cli wallet import"
programs/cli/vectrum-cli wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "vectrum-cli wallet import"
...
ACCOUNT_INFO="$(programs/cli/vectrum-cli create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "vectrum-cli create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/cli/vectrum-cli get account testera)"
verifyErrorCode "vectrum-cli get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/cli/vectrum-cli transfer inita testera 975321 "test transfer")"
    verifyErrorCode "vectrum-cli transfer"
   ...
   TRANS_INFO="$(programs/cli/vectrum-cli --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "vectrum-cli get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
