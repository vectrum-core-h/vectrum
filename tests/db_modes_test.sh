...
VECTRUM_STUFF_DIR=$(mktemp -d)
trap "rm -rf $VECTRUM_STUFF_DIR" EXIT
NODE_LAUNCH_PARAMS="./programs/node/vectrum-node -d $VECTRUM_STUFF_DIR --config-dir $VECTRUM_STUFF_DIR \
--chain-state-db-size-mb 8 --chain-state-db-guard-size-mb 0 --reversible-blocks-db-size-mb 1 \
--reversible-blocks-db-guard-size-mb 0 -e -p eosio"
