...
echo ''
echo '    The purpose of this test is to ensure that vectrum-node was built with compiler'
echo 'optimizations enabled. While there is no way to programmatically determine that'
echo 'given one binary, we do set a debug flag in vectrum-node when it is built with'
echo 'asserts. This test checks that debug flag. Anyone intending to build and install'
echo 'vectrum-node from source should perform a "release build" which excludes asserts and'
...
if [[ ! -f vectrum-node ]]; then
    echo 'ERROR: vectrum-node binary not found!'
    echo ''
    echo 'I looked here...'
...
# run vectrum-node to generate state files
./vectrum-node --extract-build-info build-info.json 1>/dev/null 2>/dev/null
