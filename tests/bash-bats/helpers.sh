...
@test "${TEST_LABEL} > previous install prompt" {
  ...
  [[ -z $(echo "${output}") ]] || exit
  # Exists, prompt
  mkdir -p $VECTRUM_INSTALL_DIR
  run previous-install-prompt
  [[ ! -z $(echo "${output}" | grep "VECTRUM has already been installed into ${VECTRUM_INSTALL_DIR}") ]] || exit
  rm -rf $VECTRUM_INSTALL_DIR
}
