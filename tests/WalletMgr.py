...
class WalletMgr(object):
    __walletLogOutFile="test_vectrum-wallet_out.log"
    __walletLogErrFile="test_vectrum-wallet_err.log"
    __walletDataDir="test_wallet_0"
    __MaxPort=9999

    # pylint: disable=too-many-arguments
    # walletd [True|False] True=Launch wallet(vectrum-wallet) process; False=Manage launch process externally.
    ...
    def launch(self):
        if not self.walletd:
            Utils.Print("ERROR: Wallet Manager wasn't configured to launch vectrum-wallet")
            return False
        ...
        # Give vectrum-wallet time to warm up
        time.sleep(2)
    ...
    def killall(self, allInstances=False):
        """Kill vectrum-wallet instances. allInstances will kill all vectrum-wallet instances running on the system."""
        if self.__walletPid:
