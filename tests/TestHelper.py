...
class TestHelper(object):
    ...
    def parse_args(includeArgs, applicationSpecificArgs=AppArgs()):
        ...
        if "--kill-count" in includeArgs:
            parser.add_argument("--kill-count", type=int, help="vectrum-node instances to kill", default=-1)
        ...
        if "--clean-run" in includeArgs:
            parser.add_argument("--clean-run", help="Kill all vectrum-node and vectrum-cli instances", action='store_true')
        if "--sanity-test" in includeArgs:
            parser.add_argument("--sanity-test", help="Validates vectrum-node and vectrum-cli are in path and can be started up.", action='store_true')
    ...
    def printSystemInfo(prefix):
        ...
        Utils.Print("vectrum-cli version: %s" % (clientVersion))
